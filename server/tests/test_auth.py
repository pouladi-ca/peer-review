...
from conftest import PASSWORD, same_origin
...
def test_login_sets_cookie_and_me_works(anon: TestClient) -> None:
    assert anon.post("/api/login", json={"password": "wrong"}, headers=same_origin()).status_code == 401
    ok = anon.post("/api/login", json={"password": PASSWORD}, headers=same_origin())
...
def test_lockout_after_five_failures(anon: TestClient) -> None:
    for _ in range(5):
        anon.post("/api/login", json={"password": "nope"}, headers=same_origin())
    blocked = anon.post("/api/login", json={"password": PASSWORD}, headers=same_origin())
